...
app.post("/order", middleware, async (req, res) => {
  ...
  try {
    ...
    if (error.message === "Insufficient USD balance") {
      ...
      error.message === "Insufficient liquidity at limit"
    ) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      const rows = await tx.$queryRaw<
        { id: string; resolution: "Yes" | "No" | null }[]
      >`SELECT id, resolution FROM "Market" WHERE id=${parsed.data.marketId} FOR UPDATE`;
      const market = rows[0];
      if (!market) throw new Error("Market not found");
      if (market.resolution) throw new Error("Market already resolved");
      const winners = await tx.position.findMany({
        where: { marketId: market.id, type: parsed.data.resolution },
      });
      for (const winner of winners) {
        await tx.user.update({
          where: { id: winner.userId },
          data: { usdBalance: { increment: winner.qty * 100 } },
        });
      }
    ...
    res
      .status(status)
      .json({ message: error.message ?? "Unable to resolve market" });
  }
});
